...
export default function UltimateEntityOneToOneRelation(
    relationEntity: ClassConstructor<any>,
    data?: Data | undefined
): PropertyDecorator {
    return function (target: Object, key: string | symbol) {
        console.log("[medusa-plugin-ultimate](ultimate-entity-relation-one-to-one):", "from", target.constructor.name, "to", relationEntity.name, `(${snakeCase(relationEntity.name)})`);

        Reflect.defineMetadata(
            ULTIMATE_ENTITY_RELATION_METADATA_KEY_NAME,
            {
                type: UltimateEntityRelationTypes.ONE_TO_ONE_RELATION_SELECT,
